...
PRINTF_HOST void putchar_(char character)
{
  exit(EXIT_FAILURE);
}
...
{
  switch(which) {
  case invokable::sprintf_:   *result = sprintf_(buffer, format, args...); break;
  case invokable::snprintf_:  *result = snprintf_(buffer, buffer_size, format, args...); break;
...
PRINTF_HD static void vfctprintf_builder_1(out_fct_type f, void* f_arg, ...)
{
  va_list args;
  va_start(args, f_arg);
...
{
  sv_and_pos svnp { {buffer, base_buffer_size}, 0 };
  vfctprintf_builder_1(append_to_buffer, &svnp, 0x12EFU);
}
